...
- Built a production-grade RAG document intelligence assistant using React, FastAPI, Celery, Redis, PostgreSQL, Qdrant, Docker, and OpenAI.
- Implemented asynchronous document ingestion so large uploads can be parsed, chunked, embedded, and indexed without blocking API requests.
- Designed provider-aware Qdrant vector collections to prevent embedding dimension mismatches between OpenAI and local fallback embeddings.
- Developed streamed RAG chat responses with citation cards, latency metadata, retrieval mode tracking, and non-streaming fallback behavior.
- Persisted document, chunk, task, chat, citation, and evaluation metadata in PostgreSQL for traceable application workflows.
- Added a Docker Compose local deployment with Swagger docs, Qdrant dashboard, and MLflow evaluation foundation.
